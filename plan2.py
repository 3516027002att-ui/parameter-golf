"""
`train_gpt.py` 和 `train_gpt_mlx.py` 脚本是供新参与者入门的好起点，而非SOTA配置。
我们接受对这些脚本进行调优、改进或简化的PR，但不能显著增加复杂度；
有竞争力的提交应放在 `/records` 文件夹中。

硬性限制：为了让新手易于阅读，`train_gpt.py` 和 `train_gpt_mlx.py` 的代码行数不得超过1500行。
"""
...
# -----------------------------
# 默认简单基线运行配置：
# - 9个transformer块，宽度512
# - 8个注意力头，4个KV头（GQA），MLP扩展2倍
# - 词汇表大小1024，序列长度1024，共享嵌入
# - 每步524,288个训练token，共20,000次迭代，约10分钟上限


class Hyperparameters:
    # 数据路径是现有预处理流程生成的分片文件通配符。
    ...
    # 验证频率和批次大小。验证始终使用完整的fineweb_val分割。
...
#
# 借鉴自 modded-nanogpt
# Muon 背景介绍：https://kellerjordan.github.io/posts/muon/


def zeropower_via_newtonschulz5(
    G: Tensor, steps: int = 10, eps: float = 1e-7
) -> Tensor:
    # 使用快速Newton-Schulz迭代对2D更新矩阵进行正交化。
    # Muon使用此方法来归一化矩阵形状的梯度，然后再应用它们。
...
# -----------------------------
#
# 小模型通常有很大比例的参数是嵌入层，因为 2 * d_model * d_vocab 的向量可能非常庞大。
# 我们不锁定分词器，而是允许你使用自己的分词器，并根据验证集的平均压缩率计算验证指标。
# 我们计算 BPB（每字节比特数）而不是验证损失，因此需要方法来统计分词器中每个token的比特数。
# 注意：编辑分词器的提交将受到更仔细的审查，因为搞砸这可能会不公平地提高你的分数。
...
def eval_val(
    ...
    # 验证计算两个指标：
    # - val_loss: token交叉熵（自然对数）
    # - val_bpb: 比赛使用的分词器无关压缩指标
...
# -----------------------------
#
# 以bf16和fp32训练的模型，以相同精度导出是愚蠢的。
# 相反，我们通过将模型量化到int8并使用zlib压缩，得到大致相同的模型（略有损失）。
# 这样我们可以在大小限制内，解压缩模型并以更高精度运行评估。
...
def quantize_float_tensor(t: Tensor) -> tuple[Tensor, Tensor]:
    t32 = t.float()
    if t32.ndim == 2:
        # 矩阵每行一个缩放因子，这通常比单个张量级缩放更能跟踪输出通道范围。
        clip_abs = (
...
def quantize_state_dict_int8(state_dict: dict[str, Tensor]):
    ...
    for name, tensor in state_dict.items():
        ...
        # FP16 embedding passthrough: tok_emb 是最敏感的张量，int8 量化会导致 ~0.007 bpb 退化
        if "tok_emb" in name:
        ...
        # 小浮点张量可以直接保留。我们仍将fp32/bf16透传张量下采样到fp16，
        # 这样元数据不会占据主要大小。
...
class TokenStream:
    # 顺序读取分片并无限循环。因此训练循环具有确定、简单的流式行为，没有采样或工作进程。
...
class DistributedTokenLoader:
    # 每次调用从共享token流中消耗一个连续块，然后为每个rank切分出一个不相交的片段。
    # 额外的"+1"个token让我们可以通过偏移来构建(x, y)。
...
class CastedLinear(nn.Linear):
    # 将权重保持在fp32以保证优化器/状态质量，在矩阵乘法时转换为bf16进行计算。
    def forward(self, x: Tensor) -> Tensor:
...
def restore_low_dim_params_to_fp32(module: nn.Module) -> None:
    # 即使模型主体在bf16中运行，也将小/控制参数保持在fp32。
    with torch.no_grad():
...
def main() -> None:
    ...
    # 优化器分组：
    # - token嵌入层（Adam）使用 EMBED_LR
    # - 非共享的lm_head（Adam）使用 HEAD_LR
    # - transformer块中的矩阵参数通过Muon使用 MATRIX_LR
    # - 向量/标量通过Adam使用 SCALAR_LR
    ...
    # 预热会初始化编译好的前向/后向/优化器路径，然后我们恢复初始权重/优化器状态，
    # 这样测量的训练就会从真实的初始状态开始。
    ...
    while True:
        ...
        grad_norm_val = torch.nn.utils.clip_grad_norm_(
            base_model.parameters(), max_norm=float("inf")
        ).item()
    ...
    # 序列化 + 往返验证
    # -----------------------------
    # 保存原始状态（用于调试/直接在PyTorch中加载），然后始终生成压缩的int8+zlib产物并验证往返的权重。
