...
class Hyperparameters:
    ...
    warmdown_iters = int(os.environ.get("WARMDOWN_ITERS", 900))
    ...
    muon_momentum_warmup_steps = int(os.environ.get("MUON_MOMENTUM_WARMUP_STEPS", 450))
...
PLAN_NAME = "plan3d"
PLAN_LOGFILE = "logs/plan3d_v1.txt"
PLAN_BUNDLE_SUMMARY = (
    "tok_emb_fp16:1 muon_wd:0.04 mlp_mult:3 "
    "bigram_hash_buckets:4096 smeargate:1 mixed_quant:int5_mlp_int6_attn"
)
...
FP16_PASSTHROUGH_PATTERNS = tuple(
    pattern
    for pattern in os.environ.get(
        "FP16_PASSTHROUGH_PATTERNS", "tok_emb,bigram_hash"
    ).split(",")
    if pattern
)
...
def classify_quant_category(name: str) -> str:
...
def mixed_quantize_int5_int6(
    ...
    for name, tensor in state_dict.items():
        ...
        category = classify_quant_category(name)
        ...
        if any(pattern in name for pattern in FP16_PASSTHROUGH_PATTERNS):
            result[name] = keep_float_tensor(name, t, passthrough_orig_dtypes)
            meta[name] = "passthrough_fp16"
            continue
        if t.numel() <= INT8_KEEP_FLOAT_MAX_NUMEL:
            result[name] = keep_float_tensor(name, t, passthrough_orig_dtypes)
            meta[name] = "passthrough_small"
            continue
        if category in {"mlp", "attn"} and t.ndim >= 1:
            clip = 15 if category == "mlp" else 31
            q, s = quantize_intN_per_row(t, clip_range=clip)
            result[name + ".q"] = q
            result[name + ".scale"] = s
            meta[name] = {"type": "int5" if category == "mlp" else "int6"}
            continue
...
def dequantize_mixed_int5_int6(
    ...
    for name, orig in template_sd.items():
        ...
        if s.ndim > 0:
            ...
            out[name] = (q.float() * float(s.item())).to(orig_dtype).contiguous()
    return out
...
def quantize_state_dict_int8(state_dict: dict[str, Tensor]):
    ...
    for name, tensor in state_dict.items():
        ...
        # tok_emb is especially sensitive to post-training quantization, so we keep
        # selected tensors in fp16 for export while restoring their original dtype on load.
        if any(pattern in name for pattern in FP16_PASSTHROUGH_PATTERNS):
...
class Rotary(nn.Module):
    ...
    def forward(
        ...
        return self._cos_cached.to(dtype=dtype).clone(), self._sin_cached.to(
            dtype=dtype
        ).clone()
...
class BigramHash(nn.Module):
    ...
    def forward(self, input_ids: Tensor) -> Tensor:
        ...
        hashed = (
            (prev_ids.long() * 92_821 + input_ids.long()) % self.num_buckets
        ).long()
...
def main() -> None:
    ...
    log0(f"plan_name:{PLAN_NAME}")
    log0(f"plan_bundle:{PLAN_BUNDLE_SUMMARY}")
    ...
    quant_result, quant_meta = mixed_quantize_int5_int6(sd_cpu)
    ...
    if master_process:
        ...
        code_bytes = len(code.encode("utf-8"))
        log0(
            f"Serialized model mixed_int5_int6+zlib: {quant_file_bytes} bytes "
            f"(raw_torch:{quant_raw_bytes})"
        )
        log0(
            f"Total submission size mixed_int5_int6+zlib: {quant_file_bytes + code_bytes} bytes"
        )
    ...
    base_model.load_state_dict(
        dequantize_mixed_int5_int6(quant_state["w"], quant_state["m"], sd_cpu),
        strict=True,
    )
    torch.cuda.synchronize()
    ...
    torch.cuda.synchronize()
    log0(
        f"final_mixed_int5_int6_zlib_roundtrip val_loss:{q_val_loss:.4f} val_bpb:{q_val_bpb:.4f} "
        f"eval_time:{1000.0 * (time.perf_counter() - t_qeval):.0f}ms"
    )
    log0(
        f"final_mixed_int5_int6_zlib_roundtrip_exact val_loss:{q_val_loss:.8f} val_bpb:{q_val_bpb:.8f}"
    )
